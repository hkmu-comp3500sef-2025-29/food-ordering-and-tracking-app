...
morgan.token('response-time-colored', (_req: Request, res: Response) => {
    ...
    if (responseTime < 100) return `\x1b[32m${responseTime.toFixed(2)}ms\x1b[0m`; // Green
    if (responseTime < 500) return `\x1b[33m${responseTime.toFixed(2)}ms\x1b[0m`; // Yellow
...
export const httpLogger = morgan(
    logFormat,
    {
        skip: (req: Request) => {
            ...
            return req.url.startsWith('/static') && process.env.NODE_ENV !== 'production';
        }
    }
);
