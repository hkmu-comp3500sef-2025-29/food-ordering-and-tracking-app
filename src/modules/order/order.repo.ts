...
type OrderDishStatus = 'placed' | 'confirmed' | 'preparing' | 'refund' | 'ready' | 'delivered';
...
function normalizeDishItem(input: OrderDishInput): OrderDishItem {
    ...
    if (input.dish_id instanceof ObjectId) dishObjectId = input.dish_id;
        ...
        if (!ObjectId.isValid(String(input.dish_id))) throw new Error('Invalid dish_id');
...
export async function createOrder(params: OrderParam[]): Promise<OrderDocument> {
...
export async function findOrder(params: OrderParam[]): Promise<OrderDocument | null> {
...
export async function findOrders(params: OrderParam[]): Promise<OrderDocument[]> {
        ...
        if (Array.isArray(q._id)) {
            ...
            for (const id of q._id) {
                if (id instanceof ObjectId) ids.push(id);
                    ...
                    if (!ObjectId.isValid(String(id))) throw new Error('Invalid MongoDB ObjectId in query');
            ...
            if (q._id instanceof ObjectId) filter._id = q._id;
                ...
                if (!ObjectId.isValid(String(q._id))) throw new Error('Invalid MongoDB ObjectId in query');
        ...
        if (Array.isArray(q.session)) {
            ...
            for (const s of q.session) {
                if (s instanceof ObjectId) sids.push(s);
                    ...
                    if (!ObjectId.isValid(String(s))) throw new Error('Invalid session id in query');
                    sids.push(new ObjectId(String(s)));
                }
            }
            if (sids.length > 0) filter.session = { $in: sids };
        } else {
            if (q.session instanceof ObjectId) filter.session = q.session;
            else {
                if (!ObjectId.isValid(String(q.session))) throw new Error('Invalid session id in query');
        ...
        if (Array.isArray(q.dish_id)) {
            ...
            for (const d of q.dish_id) {
                if (d instanceof ObjectId) dids.push(d);
                    ...
                    if (!ObjectId.isValid(String(d))) throw new Error('Invalid dish_id in query');
                    dids.push(new ObjectId(String(d)));
                }
            }
            if (dids.length > 0) filter['dish.dish_id'] = { $in: dids };
        } else {
            if (q.dish_id instanceof ObjectId) filter['dish.dish_id'] = q.dish_id;
            else {
                if (!ObjectId.isValid(String(q.dish_id))) throw new Error('Invalid dish_id in query');
    ...
    if (q.dish_status !== undefined) {
        if (Array.isArray(q.dish_status)) {
            if (q.dish_status.length > 0) filter['dish.status'] = { $in: q.dish_status };
        } else {
...
export async function updateOrder(params: OrderParam[], updates: Partial<OrderDocument>): Promise<OrderDocument | null> {
    const query: Partial<OrderDocument> = {};
    for (const p of params) {
        try {
            await p(query);
...
export async function deleteOrder(params: OrderParam[]): Promise<{ deletedCount?: number }> {
    const query: Partial<OrderDocument> = {};
    for (const p of params) {
        try {
            await p(query);
...
export async function addDishToOrder(params: OrderParam[], item: OrderDishInput): Promise<OrderDocument | null> {
...
export async function updateOrderDish(orderParams: OrderParam[], dishParams: DishItemParam[], updates: Partial<OrderDishInput>): Promise<OrderDocument | null> {
    ...
    if (updates.dish_id !== undefined) {
        ...
        throw new Error('Updating dish_id of an order dish item is not supported');
    }
    if (updates.quantity !== undefined) {
        updates.quantity = z.number().int().positive().parse(updates.quantity);
    }
    if (updates.status !== undefined) {
        const allowed: OrderDishStatus[] = ['placed', 'confirmed', 'preparing', 'refund', 'ready', 'delivered'];
    ...
    for (const d of doc.dish) {
        ...
        for (const [key, value] of Object.entries(criteria)) {
            ...
            if (key === 'dish_id') {
                let critId: ObjectId;
                if (value instanceof ObjectId) critId = value; else {
                    if (!ObjectId.isValid(String(value))) { match = false; break; }
                    critId = new ObjectId(String(value));
                }
                if (!(d.dish_id && d.dish_id.equals && d.dish_id.equals(critId))) { match = false; break; }
            } else {
                // simple equality for other fields
                const dishRecord = d as unknown as Record<string, unknown>;
                if (dishRecord[key] !== value) { match = false; break; }
            }
        }
        if (!match) continue;
...
export async function removeDishFromOrder(params: OrderParam[], dishId: string | ObjectId): Promise<OrderDocument | null> {
    const doc = await findOrder(params);
    if (!doc) return null;
    ...
    if (dishId instanceof ObjectId) targetId = dishId;
        ...
        if (!ObjectId.isValid(String(dishId))) throw new Error('Invalid dishId');
        targetId = new ObjectId(String(dishId));
    }

    const initialLength = doc.dish.length;
    const filtered = doc.dish.filter((d) => !(d.dish_id && d.dish_id.equals && d.dish_id.equals(targetId)));
...
export const WithMongoId = (id: string | ObjectId): OrderParam => WithMongoIdGeneric<OrderDocument>(id);
...
export const WithSessionId = (sessionId: string | ObjectId): OrderParam => {
    return async (config: Partial<OrderDocument>): Promise<void> => {
        ...
        if (sessionId instanceof ObjectId) sid = sessionId;
            ...
            if (!ObjectId.isValid(String(sessionId))) throw new Error('Invalid sessionId');
...
export const WithDishItems = (items: OrderDishInput[]): OrderParam => {
    return async (config: Partial<OrderDocument>): Promise<void> => {
        ...
        Object.assign(config, { dish: dishArray });
    };
};
...
export const WithDishNotes = (notes: string): DishItemParam => {
    return async (config: Partial<OrderDishInput>): Promise<void> => {
        return WithField<OrderDishInput, 'customer_notes'>('customer_notes', notes)(config);
    };
};
