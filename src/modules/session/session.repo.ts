...
export async function createSession(params: SessionParam[]): Promise<SessionDocument> {
    // initial config
    ...
    try {
        ...
        if (config.table) {
            ...
            const active = await SessionModel.findOne({ table: tableId, status: 'active' }).session(session).exec();
            if (active) {
                throw new Error('Requested table is already active in another session');
            }

            const updated = await Table.findOneAndUpdate({ _id: tableId, available: true }, { $set: { available: false } }, { session, new: true }).exec();
            if (!updated) {
                throw new Error('Requested table is not available');
            }
        } else {
            const picked = await Table.findOneAndUpdate({ available: true }, { $set: { available: false } }, { session, new: true }).exec();
            if (!picked) {
...
export async function closeSession(params: SessionParam[]): Promise<SessionDocument | null> {
    ...
    const doc = await SessionModel.findOneAndUpdate(query as any, updates, { new: true }).exec();
    if (!doc) return null;
    return doc as SessionDocument;
}


export async function findSession(params: Param<SessionDocument>[]): Promise<SessionDocument | null> {
    const query: Partial<SessionDocument> = {};
    for (const param of params) {
        try {
            await param(query);
...
export async function updateSession(params: Param<SessionDocument>[], updates: Partial<SessionDocument>): Promise<SessionDocument | null> {
    const query: Partial<SessionDocument> = {};
    for (const param of params) {
        try {
            await param(query);
...
export async function deleteSession(params: Param<SessionDocument>[]): Promise<boolean> {
    const query: Partial<SessionDocument> = {};
    for (const param of params) {
        try {
            await param(query);
...
export const WithMongoId = (id: string | ObjectId): SessionParam => WithMongoIdGeneric<SessionDocument>(id);
