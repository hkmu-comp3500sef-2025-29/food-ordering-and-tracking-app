...
export const WithApiKey = (apiKey: string): StaffParam => {
  ...
  return async (config: Partial<StaffDocument>): Promise<void> => {
    if (config.apiKey && !Array.isArray(config.apiKey)) {
      throw new Error('Invalid config.apiKey type, expected array or undefined');
    }
    if (!config.apiKey) {
      config.apiKey = [];
    }
...
export const WithMongoId = (id: string | ObjectId): StaffParam => WithMongoIdGeneric<StaffDocument>(id);
  ...
  return async (config: Partial<StaffDocument>): Promise<void> => {
    ...
    for (const id of ids) {
      if (id instanceof ObjectId) {
        ...
        if (!ObjectId.isValid(id)) throw new Error('Invalid MongoDB ObjectId in ids');
...
export async function findStaff(params: StaffParam[]): Promise<StaffDocument | null> {
...
*/
export async function findStaffs(params: StaffParam[]): Promise<StaffDocument[]> {
  ...
  const escaped = s.replace(/[^\w-]/g, '\\$&');
    ...
    if (regexes.length > 0) filter.name = { $in: regexes };
  }

  // role: single exact, array -> $in
  if (q.role !== undefined) {
    if (Array.isArray(q.role)) {
      filter.role = { $in: q.role };
    } else {
      filter.role = q.role;
    }
  }
