...
tableSchema.pre('save', async function (next) {
  ...
  try {
    if (doc.isNew && (doc.tableId === undefined || doc.tableId === null)) {
      const agg = await mongoose.connection.collection(TABLE_COLLECTION_NAME).aggregate([
        { $group: { _id: null, max: { $max: '$tableId' } } }
      ]).toArray();
      const currentMax = (agg && agg[0] && typeof agg[0].max === 'number') ? agg[0].max : 0;
...
type TableDocument = mongoose.InferSchemaType<typeof tableSchema> & mongoose.Document;
